...
const AIDoctorMatchOutputSchema = z.object({
  ...
  preliminaryAdvice: z
    .string()
    .describe('نصائح أولية مفصلة وعملية للمريض. يجب أن تتضمن خطوات محددة يمكن للمريض اتخاذها للتخفيف من الأعراض، مع التأكيد على أنها لا تغني عن الاستشارة الطبية.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `أنت مساعد طبي افتراضي وخبير في توجيه المرضى. مهمتك هي تحليل شكوى المريض واختيار الطبيب الأنسب من قائمة محددة وتقديم نصيحة أولية مفصلة.
...
5.  **صياغة نصائح أولية مفصلة:**
    - بناءً على الأعراض، قدم نصائح عملية ومفصلة يمكن للمريض اتخاذها للتخفيف من حدة المشكلة حتى يتمكن من زيارة الطبيب.
    - يمكن أن تشمل النصائح: (الراحة، شرب السوائل، وضعيات معينة، كمادات باردة أو دافئة، تجنب أطعمة أو أنشطة معينة).
    - **يجب أن تكون النصيحة آمنة تمامًا، ولا تتضمن وصف أي أدوية، وتنتهي دائمًا بجملة تؤكد على ضرورة استشارة الطبيب المختص.**
    - اجعل النصائح على شكل نقاط واضحة لتسهيل القراءة.
